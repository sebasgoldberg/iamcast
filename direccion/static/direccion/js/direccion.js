function cargar_valores_select_dependiente(select_padre,id_select_dependiente,url)
{
  if (select_padre.value)
  {
    django.jQuery.get(url,function(data,status){
      django.jQuery('#'+id_select_dependiente).html(data);
      });
  }
};
...
function change_pais(select_pais)
{
  ...
  django.jQuery('#'+id_select_estado).html('<option value="">---------</option>');
  id_select_ciudad=id_select_estado.replace('estado','ciudad');
  django.jQuery('#'+id_select_ciudad).html('<option value="">---------</option>');
